...
- **P2-4 fixed**: Added `TestEnsureHelpersRolePollutionGuard` test class to `test_hd_time_entry.py` with 3 tests that deliberately create users with polluted roles and verify each `ensure_*` helper raises `AssertionError`. All 83 tests pass (80 previous + 3 new). **NOTE (task-189 cross-reference)**: These 3 tests were subsequently moved out of `test_hd_time_entry.py` to `helpdesk/tests/test_utils.py` by task-189 (story-189 P2 fix) to co-locate them with the module they test. After that move, `test_hd_time_entry.py` contained 80 tests (not 83), and `test_utils.py` gained those 3 tests. **NOTE (task-196 cross-reference)**: commit bf2e19d09 (task-196) subsequently added 1 more test to `test_hd_time_entry.py`, bringing the current count to 81.
