...
- QA completed via API testing (curl + bench Python), unit test execution, and code review
- Playwright MCP tools were not available in this environment; all testing done via API and code analysis
- **P0 bug found:** `get_article_versions` API missing `content` field — breaks preview panel and diff view
- Fix task #244 created for the P0 issue
- All 7 unit tests pass; backend version creation, revert, and permissions all verified
- AC1 (version creation) and AC4 (revert) fully pass; AC2/AC3 partially pass (API correct, frontend broken due to missing data)

### Change Log

- 2026-03-24: Created QA report at `docs/qa-report-task-43.md`
- 2026-03-24: Created fix task #244 for P0 missing content field bug
