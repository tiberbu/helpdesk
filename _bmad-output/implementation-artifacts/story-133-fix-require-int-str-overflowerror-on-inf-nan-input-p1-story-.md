...
- **P1-1 (inf/nan string tests — da95326be, NOT cda3520c1)**: 5 string-based inf/nan rejection tests (`test_require_int_str_rejects_inf_via_stop_timer`, `test_require_int_str_rejects_infinity_string_duration`, `test_require_int_str_rejects_inf_string_as_duration_add_entry`, `test_require_int_str_rejects_negative_inf_string_duration`, `test_require_int_str_rejects_nan_string_duration`) were committed in `da95326be`, not in story-133's commit. That commit also added 1 scientific notation documentation test (`test_require_int_str_documents_scientific_notation_accepted`), 2 billable clamping tests (`test_stop_timer_clamps_billable_above_one`, `test_stop_timer_clamps_negative_billable_to_zero`), and 2 HD Admin tests (`test_hd_admin_can_stop_timer`, `test_hd_admin_can_get_summary`) — those are not inf/nan rejection tests.
- **WONTFIX — Decimal("NaN") / Decimal("Infinity") bypass in _require_int_str**: `_require_int_str` passes `Decimal("NaN")` and `Decimal("Infinity")` through unchecked because `isinstance(value, str)` is False for Decimal objects, so the string-conversion path is skipped. The float bypass guard added in `cda3520c1` only covers `float` type, not `decimal.Decimal`. This bypass is tracked but left unresolved: Frappe's HTTP layer always deserialises JSON numbers as Python `int` or `float`, never `decimal.Decimal`, so real-world callers cannot trigger this path via the API. A future task may add a `decimal.Decimal` isinstance check if the internal call surface expands.
...
- **helpdesk/helpdesk/doctype/hd_time_entry/test_hd_time_entry.py** (committed in **da95326be**, NOT cda3520c1): Added 5 string-based inf/nan rejection tests (`test_require_int_str_rejects_inf_via_stop_timer`, `test_require_int_str_rejects_infinity_string_duration`, `test_require_int_str_rejects_inf_string_as_duration_add_entry`, `test_require_int_str_rejects_negative_inf_string_duration`, `test_require_int_str_rejects_nan_string_duration`), 1 scientific notation documentation test, 2 billable clamping tests, and 2 HD Admin tests (`test_hd_admin_can_stop_timer`, `test_hd_admin_can_get_summary`).
