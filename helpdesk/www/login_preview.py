"""
Helpdesk Login Preview Page
Admin-only route for testing the redesigned login page
...
def get_context(context):
	"""
	Build context for login_preview.html template
	Implements brand resolution (AD-09) and feature-flag check (AD-06)
	"""
	# Role gate: System Manager or HD Admin only
	if frappe.session.user == "Guest":
		frappe.throw(_("Not Found"), frappe.DoesNotExistError)

	if not (
		"System Manager" in frappe.get_roles()
		or "HD Admin" in frappe.get_roles()
	):
		frappe.throw(_("Not Found"), frappe.DoesNotExistError)
