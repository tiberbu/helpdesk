...
def is_national_level(team_names: list) -> bool:
    """Return True if any team in *team_names* has level_order >= 2 (L2/L3 national)."""
...
def get_scoped_teams_for_agent(user: str):
    ...
    Returns:
        None  — user should see ALL tickets (L2+ national level)
        list  — team names whose agent_group tickets are visible (may be empty)

    Callers must check ``result is None`` to distinguish "see all" from
    "see nothing / no hierarchy configured".

    If none of the user's teams have a support_level set, returns None to
    signal that the caller should fall back to legacy behaviour.
    """
    ...
    if not any_has_level:
        # Legacy deployment: no support levels configured — signal fallback
        return None

    # L2+ National: unrestricted view
    if is_national_level(user_teams):
        return None  # signal: see all

    # Build the scoped set
    scoped = set()
    for team in user_teams:
        ...
        if order is None:
            ...
            # L3 Engineering: only own L3 team (tickets escalated to engineering)
            scoped.add(team)
        elif order == 1:
